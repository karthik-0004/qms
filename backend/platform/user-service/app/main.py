...
from .api.scim import scim_v2_router
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Rainer User Service",
        description="User profiles, RBAC, and role management",
    ...
    async def rainer_handler(request: Request, exc: RainerException) -> JSONResponse:
        return JSONResponse(status_code=exc.status_code, content=ErrorResponse.of(
            ...
            request_id=getattr(request.state, "request_id", None)).model_dump())
    ...
    async def unhandled_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        return JSONResponse(status_code=500, content=ErrorResponse.of(
            code="INTERNAL_SERVER_ERROR", message="An unexpected error occurred").model_dump())
    ...
    app.include_router(scim_v2_router)
    return app
