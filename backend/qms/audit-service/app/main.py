"""Audit Management Service — FastAPI application factory."""
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Rainer Audit Management Service",
        description="QMS Audit Management — plans, schedules, findings, checklists",
    ...
    async def rainer_handler(request: Request, exc: RainerException) -> JSONResponse:
        return JSONResponse(
            ...
            content=ErrorResponse.of(exc.code, exc.message, exc.details, getattr(request.state, "request_id", None)).model_dump(),
        )
    ...
    async def validation_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
        ...
        return JSONResponse(
            status_code=422,
            content=ErrorResponse.of("VALIDATION_ERROR", "Validation failed", details, getattr(request.state, "request_id", None)).model_dump(),
        )
    ...
    health_router = create_health_router(check_db_health)
