"""Document Service — FastAPI application factory."""
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Rainer Document Service",
        description="QMS Document Control — lifecycle management, e-signatures, version history",
    ...
    async def rainer_handler(request: Request, exc: RainerException) -> JSONResponse:
        return JSONResponse(status_code=exc.status_code, content=ErrorResponse.of(
            ...
            getattr(request.state, "request_id", None)).model_dump())
    ...
    async def unhandled_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        return JSONResponse(status_code=500, content=ErrorResponse.of(
            "INTERNAL_SERVER_ERROR", "An unexpected error occurred").model_dump())
